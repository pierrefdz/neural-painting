...
class Renderer():
    ...
    def __init__(self, renderer='oilpaintbrush', CANVAS_WIDTH=128, train=False, canvas_color='black'):
        ...
        if self.renderer in ['markerpen']:
        ...
        elif self.renderer in ['cloudbrush']:
            self.d = 12 # xc, yc, w, h, theta, R0, G0, B0, R2, G2, B2, A
            self.d_shape = 5
            self.d_color = 6
            self.d_alpha = 1
            self.brush_vertical = cv2.imread(
                r'./brushes/cloud_vertical.png', cv2.IMREAD_GRAYSCALE)
            self.brush_horizontal = cv2.imread(
                r'./brushes/cloud_horizontal.png', cv2.IMREAD_GRAYSCALE)
            ...
            raise NotImplementedError(
                'Wrong renderer name %s (choose one from [watercolor, markerpen, oilpaintbrush, rectangle, cloudbrush] ...)'
                % self.renderer)
    ...
    def check_stroke(self):
        ...
        elif self.renderer in ['oilpaintbrush', 'cloudbrush']:
    ...
    def draw_stroke(self):
        ...
        elif self.renderer == 'cloudbrush':
            return self._draw_cloudbrush()
    ...
    def _draw_cloudbrush(self):
        ...
        if h > w:
            brush = self.brush_vertical
        else:
            brush = self.brush_horizontal
        self.foreground, self.stroke_alpha_map = utils.create_transformed_brush(
            brush, self.CANVAS_WIDTH, self.CANVAS_WIDTH,
            x0, y0, w, h, theta, R0, G0, B0, R2, G2, B2)
